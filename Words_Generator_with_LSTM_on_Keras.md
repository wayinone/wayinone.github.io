...
#### The big picture:
 ...
 - The $t^{th}$ LSTM cell will produce two ouputs: the **output unit**, $h_t$, and the **cell state unit**, $c_t$, both of them are `(m,)` vectors.
 - The $t^{th}$ LSTM cell will have two inputs: the **previous output unit**, $h_{t-1}$, and **current input unit**, $x_t$
 - The inputs will generate 4 internal units (go through 4 gates), where
   * 3 of the internal units is used to generate **cell state unit** $c_t$, and
   * 1 of the internal unites and the cell state unit generate the **output unit**, $h_t$.
...
$$\bar{c}_t   = tanh(U_c\cdot h_{t-1}+W_c \cdot x_t+b_c)$$
...
where $\sigma$ is the activation function, and       
 ...
 - $b = (b_f,b_c,b_o,b_i)$, where
   ...
   - $\bar{c}$: index about **temporary cell state gate**
